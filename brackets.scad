...
// Two C-channel brackets on the panel's SHORT sides (top + bottom). Friction-fit in the frame,
// press the LCD forward onto the glass, and center the VIEW (active) area in the frame.
...
lip_z   = 1.0;        // front lip thickness (on glass, over border only)
slot_z  = panel_th + 0.3;   // panel-edge slot height
lip_y   = 3.0;        // lip / back-wall overhang onto the panel border
...
module bracket(side) {                 // side = +1 top, -1 bottom
  ...
  difference() {
    union() {
      boxc(-lx, lx, wlo, whi, 0, total_z);              // web / gap filler
      boxc(-lx, stopL, llo, lhi, 0, total_z);           // end stop L
      boxc(stopR, lx, llo, lhi, 0, total_z);            // end stop R
      boxc(stopL, stopR, llo, lhi, 0, lip_z);           // front lip
      boxc(stopL, stopR, llo, lhi, flange_z0, total_z); // back wall
    }
